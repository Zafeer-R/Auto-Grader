# M001-vd10ls: LTI 1.3 + Assignment Rendering + Numerical/Tabular Grading
...
Auto-grading system for physics lab assignments, integrated into Canvas LMS as an LTI 1.3 tool. Students complete assignments interactively inside Canvas, submissions are auto-graded, scores posted to gradebook via AGS with manual posting policy (held until instructor release).
...
- Live dependencies involved: Canvas LTI 1.3 (OIDC, JWKS), PostgreSQL, AGS grade passback
...
- A student can launch Lab01 from Canvas, fill in all gradeable fields, submit, and receive a correct score breakdown
- A TA can verify checkpoint items and the score updates accordingly
- Grades post to Canvas gradebook via AGS with manual posting policy
- A second assignment format (Lab08 pre-lab) renders and grades correctly using the same engine
...
- YAML -- less tooling support in Python ecosystem
- Database-only -- harder to version/review, no file-level diffing
...
**Alternatives Considered:**
- Celery/RQ task queue -- unnecessary overhead for deterministic grading at UTD scale
...
**Alternatives Considered:**
- Auto-deduct for inconsistency -- instructor explicitly rejected this
...
- `Assignments/Lab01_Report.docx` -- Lab 1 full report format (source material for answer key schema)
- `Assignments/Lab08_Pre-lab.docx` -- Lab 8 pre-lab format (second assignment type)
...
Per-slice acceptance criteria will be defined during slice planning. At the milestone level:
